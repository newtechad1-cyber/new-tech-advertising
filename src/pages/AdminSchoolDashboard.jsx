      ...
      try {
        ...
        submissionsData.forEach(s => {
          if (s.updated_date) activities.push({
            type: 'submission', icon: '📤', title: s.submission_title,
            status: s.status,
        ...
        storiesData.slice(0, 3).forEach(story => {
          activities.push({
            type: 'story', icon: '📖', title: story.title,
            status: story.status,
            priority: story.status === 'draft' ? 1 : 2,
            date: new Date(story.updated_date || story.created_date),
          });
        });
        activities.sort((a, b) => a.priority !== b.priority ? a.priority - b.priority : b.date - a.date);
    ...
    <div>
      <div className="bg-gradient-to-br from-blue-700 via-blue-600 to-purple-700 text-white py-12 md:py-16 mb-8 md:mb-10 rounded-3xl relative overflow-hidden shadow-xl">
        ...
        <div className="relative z-10 px-6 md:px-8">
          ...
          <p className="text-blue-100 text-lg md:text-xl font-semibold">Your content platform at a glance.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-10 md:mb-14">
        <StatCard icon={AlertCircle} label="Submissions to Review" value={stats?.pendingSubmissions || 0} color="#ef4444" route={submissionsUrl} />
        <StatCard icon={Clock} label="Projects In Progress" value={stats?.projectsInProgress || 0} color="#f59e0b" route={projectsUrl} />
        <StatCard icon={CheckCircle2} label="Stories Live" value={stats?.storiesPublished || 0} color="#10b981" route={storyLibraryUrl} />
        <StatCard icon={Video} label="Videos Live" value={stats?.videosPublished || 0} color="#3b82f6" route={videoLibraryUrl} />
      </div>

      <div className="mb-6 text-xs md:text-sm font-black text-gray-800 uppercase tracking-wider">📚 Other Content</div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-10 md:mb-14">
        <StatCard icon={BookOpen} label="Yearbook Pages" value={stats?.yearbookPages || 0} color="#8b5cf6" route={yearbookUrl} />
        <StatCard icon={BarChart3} label="Upcoming Events" value={stats?.upcomingEvents || 0} color="#ec4899" route={eventsUrl} />
        <StatCard icon={Zap} label="AI Jobs Queued" value={stats?.aiJobsPending || 0} color="#f59e0b" route={aiLabUrl} />
        <StatCard icon={AlertTriangle} label="Render Issues" value={stats?.failedRenders || 0} color="#dc2626" route={videoLibraryUrl} />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-amber-500 order-1 md:order-2">
          <h3 className="text-lg md:text-xl font-bold mb-1 flex items-center gap-2">
            <AlertTriangle className="h-5 w-5 text-amber-600" /> Action Items
          </h3>
          <p className="text-sm text-gray-600 mb-4">Tasks that need your attention</p>
          <div className="space-y-2">
            {stats?.pendingSubmissions > 0 && (
              <Link to={submissionsUrl} className="block p-3 bg-red-50 hover:bg-red-100 rounded-lg border border-red-200 transition-colors">
            ...
            {stats?.aiJobsPending > 0 && (
              <Link to={aiLabUrl} className="block p-3 bg-yellow-50 hover:bg-yellow-100 rounded-lg border border-yellow-200 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
            ...
            {stats?.failedRenders > 0 && (
              <Link to={videoLibraryUrl} className="block p-3 bg-orange-50 hover:bg-orange-100 rounded-lg border border-orange-200 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
      ...
      <div className="mt-10 md:mt-14 bg-gradient-to-br from-blue-50 via-purple-50 to-blue-50 rounded-3xl p-8 md:p-10 border-2 border-blue-200 shadow-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          <Link to={submissionsUrl} className="group bg-white hover:bg-red-50 p-6 md:p-8 rounded-2xl text-center border-2 border-red-100 hover:border-red-300 transition-all hover:shadow-lg hover:-translate-y-2">
          ...
          <Link to={projectsUrl} className="group bg-white hover:bg-blue-50 p-6 md:p-8 rounded-2xl text-center border-2 border-blue-100 hover:border-blue-300 transition-all hover:shadow-lg hover:-translate-y-2">
            <p className="text-4xl md:text-5xl mb-3">🎬</p>
          ...
          <Link to={aiLabUrl} className="group bg-white hover:bg-yellow-50 p-6 md:p-8 rounded-2xl text-center border-2 border-yellow-100 hover:border-yellow-300 transition-all hover:shadow-lg hover:-translate-y-2">
            <p className="text-4xl md:text-5xl mb-3">⚡</p>
          ...
          <Link to={storyLibraryUrl} className="group bg-white hover:bg-purple-50 p-6 md:p-8 rounded-2xl text-center border-2 border-purple-100 hover:border-purple-300 transition-all hover:shadow-lg hover:-translate-y-2">
            <p className="text-4xl md:text-5xl mb-3">📖</p>
