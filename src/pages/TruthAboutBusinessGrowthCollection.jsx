...
export default function TruthAboutBusinessGrowthCollection() {
  ...
  useEffect(() => {
    ...
    const completed = truthAboutBusinessGrowthLessons
      .filter(lesson => progress[`truth-about-business-growth:${lesson.slug}`])
  ...
  const firstIncompleteLesson = truthAboutBusinessGrowthLessons.find(l => !completedLessons.includes(l.id)) || truthAboutBusinessGrowthLessons[0];
  const primaryButtonText = isAllComplete ? "Review Collection" : completedLessons.length > 0 ? "Continue Journey" : "Start Journey";
  const primaryButtonLink = `/knowledge/truth-about-business-growth/${firstIncompleteLesson?.slug || ''}`;

  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-200">
      <SEOHead 
        title="The Truth About Business Growth | NTA Knowledge Library"
        description="A foundational collection exploring how businesses actually grow by building robust systems rather than relying on disconnected marketing campaigns."
      />
      <MarketingNav />
      ...
      <section className="pt-32 pb-16 px-6 border-b border-slate-800 bg-slate-900/50">
        <div className="max-w-4xl mx-auto text-center">
          <div className="flex items-center justify-center gap-2 text-sm font-medium text-slate-400 mb-8 overflow-x-auto whitespace-nowrap">
            ...
            <span className="text-slate-300">The Truth About Business Growth</span>
          </div>
          ...
          <p className="text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10">
            Learn why marketing alone isn't enough, and discover how to build connected systems that drive real, sustainable business growth.
          </p>
          ...
          <div className="max-w-md mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
            <div className="flex justify-between items-end mb-3">
              ...
              <div className="text-emerald-400 font-bold text-xl">{progressPercent}%</div>
            </div>
            ...
            <div className="mt-6">
              <Link 
                ...
                className="block w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold transition-colors text-center"
              >
                {primaryButtonText}
              </Link>
            </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
            <PlayCircle className="text-emerald-500 w-6 h-6" /> Collection Lessons
          </h2>
          
          <div className="space-y-4">
            {truthAboutBusinessGrowthLessons.map((lesson, index) => {
              const isCompleted = completedLessons.includes(lesson.id);
              const isNext = !isCompleted && (index === 0 || completedLessons.includes(truthAboutBusinessGrowthLessons[index - 1].id));
              
              return (
                <Link 
                  key={lesson.id}
                  to={`/knowledge/truth-about-business-growth/${lesson.slug}`}
                  className={`block p-6 rounded-2xl border transition-all duration-300 ${
                    isNext 
                      ? 'bg-emerald-900/10 border-emerald-500/30 hover:border-emerald-500/60 shadow-[0_0_20px_rgba(16,185,129,0.05)]' 
                    ...
                    <div className="flex-shrink-0 mt-1">
                      {isCompleted ? (
                        ...
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 font-bold text-sm ${
                          isNext ? 'border-emerald-500 text-emerald-400 bg-emerald-500/10' : 'border-slate-700 text-slate-500'
                        }`}>
                          {index + 1}
                        </div>
                      )}
