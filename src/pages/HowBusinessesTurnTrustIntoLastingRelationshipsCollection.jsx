...
export default function HowBusinessesTurnTrustIntoLastingRelationshipsCollection() {
  ...
  useEffect(() => {
    ...
    const completed = howBusinessesTurnTrustIntoLastingRelationshipsLessons
      .filter(lesson => progress[`how-businesses-turn-trust-into-lasting-relationships:${lesson.slug}`])
  ...
  const firstIncompleteLesson = howBusinessesTurnTrustIntoLastingRelationshipsLessons.find(l => !completedLessons.includes(l.id)) || howBusinessesTurnTrustIntoLastingRelationshipsLessons[0];
  const primaryButtonText = isAllComplete ? "Review Collection" : completedLessons.length > 0 ? "Continue Journey" : "Start Journey";
  const primaryButtonLink = `/knowledge/how-businesses-turn-trust-into-lasting-relationships/${firstIncompleteLesson?.slug || ''}`;

  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-200">
      <SEOHead 
        title="How Businesses Turn Trust Into Lasting Relationships | NTA Knowledge Library"
        description="A cornerstone collection exploring how relationships are built and sustained after the sale."
      />
      <MarketingNav />
      ...
      <section className="pt-32 pb-16 px-6 border-b border-slate-800 bg-slate-900/50">
        <div className="max-w-4xl mx-auto text-center">
          <div className="flex items-center justify-center gap-2 text-sm font-medium text-slate-400 mb-8 overflow-x-auto whitespace-nowrap">
            ...
            <span className="text-slate-300">How Businesses Turn Trust Into Lasting Relationships</span>
          </div>
          ...
          <p className="text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10">
            Learn why the sale is just the beginning, and discover the systems that keep your business valuable to customers for years to come.
          </p>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {howBusinessesTurnTrustIntoLastingRelationshipsLessons.map((lesson, index) => {
              const isCompleted = completedLessons.includes(lesson.id);
              const isNext = !isCompleted && (index === 0 || completedLessons.includes(howBusinessesTurnTrustIntoLastingRelationshipsLessons[index - 1].id));
              
              return (
                <Link 
                  key={lesson.id}
                  to={`/knowledge/how-businesses-turn-trust-into-lasting-relationships/${lesson.slug}`}
