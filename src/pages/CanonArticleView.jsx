...
export default function CanonArticleView({ articleSlug }) {
  ...
  const related = kg.getRelatedContent(article.canon_id);
  const relatedReading = [
    ...[...related.reading, ...related.learning]
      .filter(item => item.status === 'Published')
      .map(item => ({ title: item.title, path: item.canonical_url || '/canon/' + item.slug })),
    ...journalIssues
      .filter(issue => contentPath(issue.cta_url) === contentPath(canonicalPath))
      .map(issue => ({ title: issue.title, path: '/journal/' + (issue.slug || 'issue-' + issue.issue_number) })),
  ].slice(0, 2);
      ...
      <main className="flex-grow">
        ...
        <section className="px-6 pb-12">
          <div className="mx-auto max-w-3xl">
            <ContentNextSteps title={article.title} path={canonicalPath} resources={relatedReading} />
          </div>
        </section>
