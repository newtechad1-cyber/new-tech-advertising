...
export default function ExecutiveDashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      // Try loading KnowledgeCapture
      let allKnowledge = [];
      try {
        allKnowledge = await base44.entities.KnowledgeCapture?.list() || [];
      } catch { /* entity may not exist yet */ }
      ...
      setOsHealth({
        k_series: { completed: 1, in_build: 0, pending: 'TBD' },
        e_series: { completed: 2, in_build: 1, pending: 'TBD' },
        a_series: { completed: 1, in_build: 0, pending: 'TBD' },
        m_series: { completed: 1, in_build: 0, pending: 'TBD' },
      });

      // ── Knowledge Engine Metrics ──
      const kToday = new Date().toISOString().split('T')[0];
      const kThisWeek = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
      setKnowledgeMetrics({
        capturedToday: allKnowledge.filter(k => k.created_date?.startsWith(kToday)).length,
        waitingReview: allKnowledge.filter(k => k.status === 'needs_review').length,
        approvedThisWeek: allKnowledge.filter(k => k.status === 'approved' && k.approved_date >= kThisWeek).length,
        highPriority: allKnowledge.filter(k => (k.priority === 'high' || k.priority === 'urgent') && k.status !== 'approved' && k.status !== 'archived').length,
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-4 space-y-4">
            ...
            {/* Knowledge Engine */}
            <div className="bg-gradient-to-r from-indigo-900/20 to-slate-900 border border-indigo-800/30 rounded-2xl p-4">
              <h3 className="text-indigo-400 text-xs font-semibold uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <BookOpen className="w-3.5 h-3.5" /> Knowledge Engine
              </h3>
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-slate-800/40 rounded-xl px-3 py-2 text-center">
                  <p className="text-lg font-bold text-white">{knowledgeMetrics.capturedToday || 0}</p>
                  <p className="text-xs text-slate-500">Captured Today</p>
                </div>
                <div className="bg-slate-800/40 rounded-xl px-3 py-2 text-center">
                  <p className={`text-lg font-bold ${(knowledgeMetrics.waitingReview || 0) > 0 ? 'text-yellow-400' : 'text-white'}`}>
                    {knowledgeMetrics.waitingReview || 0}
                  </p>
                  <p className="text-xs text-slate-500">Waiting Review</p>
                </div>
                <div className="bg-slate-800/40 rounded-xl px-3 py-2 text-center">
                  <p className="text-lg font-bold text-emerald-400">{knowledgeMetrics.approvedThisWeek || 0}</p>
                  <p className="text-xs text-slate-500">Approved (Week)</p>
                </div>
                <div className="bg-slate-800/40 rounded-xl px-3 py-2 text-center">
                  <p className={`text-lg font-bold ${(knowledgeMetrics.highPriority || 0) > 0 ? 'text-orange-400' : 'text-white'}`}>
                    {knowledgeMetrics.highPriority || 0}
                  </p>
                  <p className="text-xs text-slate-500">High Priority</p>
                </div>
              </div>
              <Link
                to="/admin/knowledge-capture"
                className="flex items-center justify-center gap-1.5 w-full mt-3 px-3 py-2 bg-indigo-600/20 text-indigo-400 rounded-xl text-xs font-medium hover:bg-indigo-600/30 transition"
              >
                <BookOpen className="w-3.5 h-3.5" /> Open Knowledge Engine
              </Link>
            </div>
          </div>
        ...
        <div className="grid grid-cols-7 gap-3">
          {[
            ...
            { label: 'Knowledge Capture', icon: BookOpen, link: '/admin/knowledge-capture', accent: 'bg-indigo-800 hover:bg-indigo-700 text-indigo-200' },
            { label: 'Knowledge Review', icon: Eye, link: '/admin/knowledge-review', accent: 'bg-slate-800 hover:bg-slate-700 text-slate-300' },
