...
export default function TurningWhatABusinessKnowsIntoAnAssetLesson() {
  const { slug } = useParams();
  const [isCompleted, setIsCompleted] = useState(false);

  const lessonIndex = useMemo(() => {
    return turningWhatABusinessKnowsIntoAnAssetLessons.findIndex(l => l.slug === slug);
  }, [slug]);
  ...
  useEffect(() => {
    ...
    if (slug) {
      ...
      setIsCompleted(!!progress[`turning-what-a-business-knows-into-an-asset:${slug}`]);
    }
  }, [slug]);

  const handleMarkComplete = () => {
    updateLearningProgress(`turning-what-a-business-knows-into-an-asset:${slug}`, true);
    setIsCompleted(true);
  };

  if (lessonIndex === -1) {
    return <Navigate to="/knowledge/turning-what-a-business-knows-into-an-asset" replace />;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-200">
      <SEOHead 
        title={`${lesson.title} | Turning What a Business Knows Into an Asset`}
      ...
      <main className="pt-24 pb-20">
        ...
        <header className="px-6 mb-12">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center gap-2 text-sm font-medium text-slate-400 mb-8 overflow-x-auto whitespace-nowrap">
              ...
              <Link to="/knowledge/turning-what-a-business-knows-into-an-asset" className="hover:text-white transition-colors">Turning What a Business Knows Into an Asset</Link>
              <ChevronRight className="w-4 h-4 text-slate-600" />
              <span className="text-blue-400">Lesson {currentStep}</span>
            </div>

            <TrackProgress trackName="Turning What a Business Knows Into an Asset" currentStep={currentStep} totalSteps={7} color="blue" />
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <TrackBottomNav 
            prevLink={lessonIndex > 0 ? `/knowledge/turning-what-a-business-knows-into-an-asset/${turningWhatABusinessKnowsIntoAnAssetLessons[lessonIndex - 1].slug}` : null}
            prevText="← Previous Lesson"
            nextLink={lesson.nextLessonSlug ? `/knowledge/turning-what-a-business-knows-into-an-asset/${lesson.nextLessonSlug}` : "/knowledge/turning-what-a-business-knows-into-an-asset"}
