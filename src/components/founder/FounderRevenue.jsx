...
export default function FounderRevenue() {
  const { data: transactions = [] } = useQuery({
    queryKey: ['founder-transactions'],
    queryFn: () => base44.entities.BillingTransactions.list('-created_date', 200),
  });

  const { data: invoices = [] } = useQuery({
    queryKey: ['founder-invoices'],
    queryFn: () => base44.entities.BillingInvoices.list('-created_date', 200),
  });

  const { data: financeItems = [] } = useQuery({
    queryKey: ['founder-finance'],
    queryFn: () => base44.entities.FinanceTransactions.list('-created_date', 200),
  });

  const { data: resellerRevenue = [] } = useQuery({
    queryKey: ['founder-reseller-rev'],
    queryFn: () => base44.entities.ResellerRevenue.list('-created_date', 100),
  });

  const { data: expenses = [] } = useQuery({
    queryKey: ['founder-expenses'],
    queryFn: () => base44.entities.Expenses.list('-created_date', 100),
  });
  ...
  const collected = transactions
    .filter(t => t.status === 'succeeded' && new Date(t.created_date) >= startOfMonth)
    .reduce((s, t) => s + (t.amount || 0), 0);

  const newRevenue = invoices
    .filter(i => i.status === 'paid' && new Date(i.created_date) >= startOfMonth)
    .reduce((s, i) => s + (i.amount || 0), 0);

  const failed = invoices.filter(i =>
    (i.status === 'failed' || i.status === 'past_due') && new Date(i.created_date) >= startOfMonth
  );
  const failedAmount = failed.reduce((s, i) => s + (i.amount || 0), 0);

  const pastDue = invoices.filter(i => i.status === 'past_due').length;

  const commissions = resellerRevenue
    .filter(r => new Date(r.created_date) >= startOfMonth)
    .reduce((s, r) => s + (r.commission_amount || 0), 0);

  const monthExpenses = expenses
    .filter(e => new Date(e.created_date) >= startOfMonth)
  ...
  const chartData = Array.from({ length: 6 }, (_, i) => {
    ...
    const rev = transactions
      .filter(t => {
        ...
        return t.status === 'succeeded' && td >= d && td <= monthEnd;
      })
      .reduce((s, t) => s + (t.amount || 0), 0);
