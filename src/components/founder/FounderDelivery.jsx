...
export default function FounderDelivery() {
  const { data: clients = [] } = useQuery({
    queryKey: ['founder-delivery-clients'],
    queryFn: () => base44.entities.ClientCompanies.list(),
  });

  const { data: tasks = [] } = useQuery({
    queryKey: ['founder-agent-tasks'],
    queryFn: () => base44.entities.AgentTasks.list('-created_date', 300),
  });

  const { data: runs = [] } = useQuery({
    queryKey: ['founder-agent-runs'],
    queryFn: () => base44.entities.AgentRuns.list('-created_date', 100),
  });

  const { data: activityLog = [] } = useQuery({
    queryKey: ['founder-activity-log'],
    queryFn: () => base44.entities.ClientActivityLog.list('-created_date', 500),
  });
  ...
  const stuckOnboarding = onboardingClients.filter(c =>
    new Date(c.updated_date) < threeDaysAgo
  );

  const pendingApprovals = tasks.filter(t =>
    t.status === 'pending_approval' || t.status === 'awaiting_approval'
  ).length;
  ...
  const recentActiveClientIds = new Set(
    activityLog
      .filter(a => new Date(a.created_date) >= fourteenDaysAgo)
      .map(a => a.company_id)
  );
  const inactiveClients = clients.filter(c =>
    (c.status === 'active' || !c.status) && !recentActiveClientIds.has(c.id)
  ).length;
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 h-full flex flex-col">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
        ...
        <DelivStat
          label="Stuck ≥3 Days"
          value={stuckOnboarding.length}
          color={stuckOnboarding.length > 0 ? 'text-red-400' : 'text-green-400'}
        />
        <DelivStat
          label="Pending Approvals"
          value={pendingApprovals}
          color={pendingApprovals > 0 ? 'text-yellow-400' : 'text-gray-400'}
        />
        <DelivStat label="Tasks Queued" value={queuedTasks} color="text-gray-300" />
        <DelivStat
          label="Failed Runs Today"
          value={failedRunsToday}
          color={failedRunsToday > 0 ? 'text-red-400' : 'text-green-400'}
        />
        <DelivStat
          label="No Activity 14d"
          value={inactiveClients}
          color={inactiveClients > 0 ? 'text-yellow-400' : 'text-green-400'}
        />
        <DelivStat
          label="Failed Tasks"
          value={failedTasks.length}
          color={failedTasks.length > 0 ? 'text-red-400' : 'text-green-400'}
        />
      </div>
            ...
            {failedTasks.slice(0, 8).map(t => (
              <div key={t.id} className="flex items-center justify-between text-xs bg-red-900/20 border border-red-900/30 rounded-md px-2.5 py-1.5">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-gray-500">{t.agent_type || t.task_type || '—'}</span>
                  <span className="text-red-400 font-medium">Failed</span>
                </div>
              </div>
            ))}
            {failedTasks.length > 8 && (
              <div className="text-xs text-gray-600 text-center py-1">+{failedTasks.length - 8} more</div>
            )}
          </div>
        </div>
      )}
            ...
            {stuckOnboarding.slice(0, 4).map(c => (
              <div key={c.id} className="flex items-center justify-between text-xs bg-yellow-900/20 border border-yellow-900/30 rounded-md px-2.5 py-1.5">
                ...
                <span className="text-yellow-500 flex-shrink-0">
                  {Math.floor((now - new Date(c.updated_date)) / (1000 * 60 * 60 * 24))}d stalled
                </span>
              </div>
            ))}
          </div>
