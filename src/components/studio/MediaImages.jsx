...
import { Plus, Trash2, Upload, Search, Copy } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm">{assets.length} images for social, video & blog</p>
        </div>
        <label className="cursor-pointer">
          <input type="file" accept="image/*" onChange={handleFile} className="hidden" />
          <Button asChild className="bg-pink-600 hover:bg-pink-700 pointer-events-none">
      ...
      {showForm && pendingFile && (
        <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 space-y-4">
          <p className="text-slate-300 font-medium">Save image: <span className="text-white">{pendingFile.name}</span></p>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input value={form.tags} onChange={e => setForm({...form, tags: e.target.value})} placeholder="logo, summer, promo" className="bg-slate-800 border-slate-700 text-white" />
            </div>
          </div>
          <div>
          ...
          <div className="flex gap-3">
            <Button onClick={save} disabled={uploading} className="bg-pink-600 hover:bg-pink-700">{uploading ? 'Uploading...' : 'Save Image'}</Button>
      ...
      {loading ? <p className="text-slate-400">Loading...</p> : (
        filtered.length === 0 ? (
          <div className="text-center py-16 text-slate-500">No images yet. Upload your first one!</div>
        ) : (
