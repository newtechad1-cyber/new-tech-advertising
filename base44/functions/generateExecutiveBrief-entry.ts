...
export default async function generateExecutiveBrief() {
  ...
  const [leads, audits, proposals, healthChecks, knowledgeCaptures] = await Promise.all([
    SalesLead.list().catch(() => []),
    GapAudit.list().catch(() => []),
    NtaProposal.list().catch(() => []),
    SystemHealthCheck.list().catch(() => []),
    KnowledgeCapture.list().catch(() => []),
  ]);
  ...
  // ── 7. Knowledge Engine Metrics ──
  const kThisWeek = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
  const knowledgeCapturedToday = knowledgeCaptures.filter((k: any) => k.created_date?.startsWith(today)).length;
  const knowledgeWaitingReview = knowledgeCaptures.filter((k: any) => k.status === 'needs_review').length;
  const knowledgeApprovedWeek = knowledgeCaptures.filter((k: any) => k.status === 'approved' && k.approved_date >= kThisWeek).length;
  const knowledgeHighPriority = knowledgeCaptures.filter((k: any) => (k.priority === 'high' || k.priority === 'urgent') && k.status !== 'approved' && k.status !== 'archived').length;
...
KNOWLEDGE ENGINE:
- Captures today: ${knowledgeCapturedToday}
- Waiting review: ${knowledgeWaitingReview}
- Approved this week: ${knowledgeApprovedWeek}
- High priority items: ${knowledgeHighPriority}
- Total captures: ${knowledgeCaptures.length}
...
Be specific. Use real numbers from the data. Be Rick's trusted advisor — direct, honest, helpful.`
  ...
  // ── 10. Create or update today's brief ──
  const briefData = {
    brief_date: today,
    generated_at: now,
