...
Deno.serve(async (req) => {
  try {
    ...
    let url = `https://api.freshbooks.com/accounting/account/${accountId}/users/clients`;
    if (payload.clientid) {
    ...
    if (method === "POST" || method === "PUT") {
        options.body = JSON.stringify({ client: payload });
    }
    ...
    if (method === "GET" && result.clients === undefined && Array.isArray(result.client)) {
       result = { clients: result.client };
    }
